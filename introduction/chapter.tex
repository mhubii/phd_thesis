...
According to many, the future of surgery is robotic \cite{times2021better}. This trend was already exposed in \secref{in:sec:foreword}, \figref{in:fig:robotic_vs_laparoscopic_vs_open}, where \cite{sheetz2020trends} found an average increase in robotic laparoscopy from $1.8\%$ to $15.1\%$ post robotic laparoscopy introduction. For cholecystectomy, the most common procedure, an increase from $2.5\%$ to $7.5\%$ was found. Somewhat paradoxically, laparoscopic cholecystectomy is already a routine procedure with very low complication rates, \cite{thapar2023evaluation} e.g. reports a $0.2\%$ 30 days mortality rate in India. Hence, questions about benefits of robotic laparoscopy may be raised rightfully.

\paragraph{Patient Side Aspects} More than twenty years passed since the introduction of the da Vinci\textsuperscript{\textregistered} surgical robot by Intuitive (\figref{in:fig:da_vinci}), which was launched in 1999, and many review studies compared classical vs. robotic laparoscopies ever since to provide evidence-based care. The majority of which suggests that no significant advantages exist for patients. Despite increased cost and longer intervention times, these studies report equal complication, mortality, and conversion rates (i.e. conversion to open surgery), as well as equal post operative stay duration \cite{kawka2023laparoscopic, csirzo2023robot}. Some studies, however, also highlight underrepresented patient-reported outcome measures \cite{kawka2023laparoscopic} (e.g. post-operative pain, return to function). 

\paragraph{Surgeon Side Aspects} Undoubtedly, the market for RAS, which is currently dominated by Intuitive Surgical, Inc., is growing sustainably, see \figref{in:fig:intuitive_revenue}.
\begin{figure}
    \centering
...
\paragraph{Robot-free Surgeries} Surgery cost might not only be reduced through continuous camera motion, it might also be reduced in procedures where robot assistance is currently not common practise, such as laparoscopic cholecystectomy. In Michigan, United States, e.g., $7.5\%$ of all laparoscopic cholecystectomies were performed without a robot \cite{sheetz2020trends}. The numbers are likely lower in other countries. Domains without robot dominance could initially benefit from replacing the camera holder assistant, refer \figref{in:fig:room_setup}, thus reducing the cost drastically and also allowing the assistant to perform more meaningful tasks. This could further remove the communication barrier between surgeon and camera holder, thus improving the surgeon convenience and reducing the operation time. Indeed, some companies, e.g. Moon Surgical (Maestro\textsuperscript{\textregistered}), CMR Surgical (Versius\textsuperscript{\textregistered}), and Distalmotion (Dexter\textsuperscript{\textregistered}), target this area through their modular systems. Although they do not target automation yet, economically speaking, and due to the growth potential for camera motion automation there, research should aim at this domain as well. Due to the progressive nature of healthcare \cite{chatterjee2024advancements}, it is almost certain that automation will grow in importance as robots will be deployed to laparoscopic cholecystectomy. Automating camera motion in domains without robot dominance introduces implications that will become clearer in \secref{in:sec:imitation_learning}. For now, it should only be emphasized that these domains are of particular relevance. Any laparoscopic camera motion automation attempt should be compatible with surgeries that are robotic and those that will be.

% Automation may reduce human error that is linked to fatigue, lack of attention and cognitive overload \cite{fiorini2022concepts}. It could help surgeons operate robotic systems by reducing the learning curve \cite{van2018learning}. And in an ageing society with shrinking workforce, it could help to retain accessibility to healthcare.
% build a digital twin of surgeon (CEO of Asensus Surgical, Senhance TM) \cite{zidane2023robotics}, thus improving accesibility (cost)

% \cite{chatterjee2024advancements}
...
\paragraph{System Considerations} Whilst automation can be achieved on robotic systems with mechanical RCM and systems with programmable RCM, refer \secref{in:sec:robot_surgery_platforms}, in this work, the main focus will be put on systems with programmable RCM. This is in an attempt to lower cost for RMIS drastically, deviating from the status-quo, the da Vinci\textsuperscript{\textregistered} robot, which has a mechanical RCM. Deviating from the status-quo boosts competition, which may significantly reduce the cost of RMIS. Furthermore, systems with programmable RCM are often off the shelf industrial serial arm manipulators. These are built at scales beyond surgery, which could lower costs further. Finally, systems with programmable RCM can be used flexibly across multiple types of surgeries, e.g. spine \cite{farber2021robotics} or orthopaedic surgery \cite{suarez2023revolutionizing}, thereby reducing cost even more. To facilitate the potential cost reduction that arise with programmable RCMs, methods in this thesis should therefore not be limited to, but also obey the principles of programmable RCMs. 
...
% explain the different approaches to automation their up and downsides
...
\subsection{Implications on the Clinical Workflow}
