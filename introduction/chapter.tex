...
\end{figure}
\subsubsection{Laparoscopic Cholecystectomy Setup}
...
\paragraph{Dissection of the Hepatocystic Triangle} The goal is to expose the hepatocystic trianlge \cite{mischinger2020critical}, see \figref{in:fig:hepatocystic_triangle}. First, the gallbladder is gently pulled upward over the liver, and its neck is pulled downward to expose its different parts, as can be seen in \figref{in:fig:cholecystectomy_incisions}. Swollen gallbladders are precautiously poked to prevent perforation with leakage of bile and gallstones. Potential adhesions (scarred connections to other organs) are carefully separated using cautery and regular tools, avoiding the area near the duodenum (beginning of the small intestine).


% \paragraph{Hepatocystic Triangle Dissection} The goal is to clear away fat and tissue around the gallbladder and to clear the hepatocystic triangle, see \figref{in:fig:hepatocystic_triangle}. To start, the gallbladder is gently pulled upward over the liver, and its neck is pulled downward to expose its different parts, as can be seen in \figref{in:fig:cholecystectomy_incisions} and \figref{in:fig:hepatocystic_triangle}. Swollen gallbladders are precautiously poked to prevent perforation with leakage of bile and gallstones. Potential adhesions (scarred connections to other organs) are carefully separated using cautery and regular tools, avoiding the area near the duodenum (beginning of the small intestine).

% It is of utmost importance to identify the cystic artery and cystic duct without doubt prior to 

\paragraph{Establishing the Critical View of Safety} To ensure safety, three things are checked: the area around the gallbladder's neck is free of fat and tissue, the lower part of the gallbladder is separated from the liver, and only two structures (cystic duct and cystic artery) enter the gallbladder. Once this safe view is achieved, the surgeon and assistant double-check before cutting or clipping anything. Understanding variations in anatomy is crucial to avoid mistakes.

\begin{figure}
    \centering
    \includegraphics[width=0.6\textwidth]{introduction/fig/24_02_03_hepatocystic_triangle.png}
    \caption{The bile duct (green), together with liver and gallbladder form the hepatocystic triangle, which is often covered in fat tissue. Image with courtesy of \cite{mischinger2020critical} and updated font.}
    \label{in:fig:hepatocystic_triangle}
\end{figure}

\paragraph{Clipping and Cutting the Cystic Artery} To save costs, a tool with clips is used to close the cystic artery, and then it is cut. Care is taken not to displace the clips, leaving a small piece of tissue to prevent any accidental movement during or after the procedure.

\paragraph{Checking and Cutting the Cystic Duct} An X-ray is often taken to check the ducts. If needed, the cystic duct is clipped and cut. If there are concerns about the clips, a special loop suture is used instead.

\paragraph{Separating Gallbladder from Liver} After cutting the cystic duct, the gallbladder is carefully dissected from the liver. Special tools and techniques are used to avoid bleeding or bile leakage. Any bleeding or spillage is cleaned up. The gallbladder is then put in a bag and removed through a port.

\paragraph{Removing Specimen and Closing} The bag containing the gallbladder is taken out through a port, and the skin and fascia at the extraction site are closed with sutures. All gas used during the procedure is released from the ports.

% organs: https://commons.wikimedia.org/wiki/File:Digestive_system_showing_bile_duct.svg?uselang=en#Licensing



% TODO: add a figure that shows the workspace etc
% setup figure: https://www.sages.org/image-library/basic-room-setup/
% setup figure: https://www.researchgate.net/publication/267805211_Ergonomic_considerations_in_natural_orifice_translumenal_endoscopic_surgery_NOTES_A_case_study
% explanatory video: https://ales.amegroups.org/article/view/5766/html also contains anathomy image
