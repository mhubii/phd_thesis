...
\textbf{\chapref{chap:robotic_endoscope}} proposes a novel control scheme for robotic laparoscope holders that combines \label{in:sec:homography_based_camera_motion_formulation} through a \gls{rcm} objective with a homography-based camera frame velocity task. This paradigm addresses the dominant tool following assumption of \secref{in:sec:rule_based_approaches}. The effectiveness of the proposed control scheme is demonstrated on a phantom setup in a semi-autonomous scenario (level three autonomy, \secref{in:sec:foreword}). The locality and temporality assumptions of the proposed method therein are discussed in the context of the proposed \gls{il} framework of \figref{in:fig:hypothesized_pipeline}.

\textbf{\chapref{chap:camera_motion_extraction}}  

\textbf{\chapref{chap:camera_motion_prediction}}
