...
Since the camera motion estimator is executed online, memory footprint and computational efficiency are of importance. Therefore, we evaluate the estimator from~\cite{huber2022deep} with a ResNet-34 backbone, \grl{surf} \& \grl{ransac}, and \grl{loftr}~\cite{sun2021loftr} \& \grl{ransac}. Each estimator is run 1000 times on a single image sequence of length $N+M=15$ with an NVIDIA GeForce RTX 2070 \gls{gpu} and an Intel(R) Core(TM) i7-9750H \gls{cpu} @ 2.60GHz.
...
\begin{table}[htb]
    ...
    \resizebox{\textwidth}{!}{
        \begin{tabular}{lrrr}
            ...
            \grl{loftr} \& \grl{ransac} & $0.178 \pm 0.06$ & $1.0$ & $669 / 2412$ \\
            \grl{surf} \& \grl{ransac} & $0.131 \pm 0.024$ & $1.4$ & NA \\
            \bottomrule
        \end{tabular}
    }
\end{table}
