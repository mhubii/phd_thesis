...
Having successfully established a homography-based visual servo under RCM constraint in the previous \chapref{chap:robotic_endoscope}, which is based on simple optimal control and, unlike end-to-end learned robot policies, inherently safe for clinical use, this chapter will now investigate the generation of large scale state-action pairs $(\hat{s}_t,\,\hat{a}^*_t)$. The generation of these state-action pairs is a prerequisite for extracting the human expert policy through IL methods, refer \secref{in:sec:imitation_learning}, which will be delved further into in \chapref{chap:camera_motion_prediction}. For now, the focus will solely lie on extracting actions to form state-action pairs.
...
\subsubsection{da Vinci\textsuperscript{\textregistered} Surgery Data Pre-Processing}
