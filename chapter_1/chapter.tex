...
\paragraph{Disclaimer} This \chapref{chap:registration} is an \textit{in extenso} reproduction of TODO: submitted to??. Only \secref{c2:sec:introduction} was altered to highlight additional context within the scope of this thesis.
...
The key coordinate frames are illustrated in the schematic diagrams shown in \figref{c1:fig:base2base} and \figref{in:fig:eye_in_hand}. 
...
A homogeneous transformation matrix, denoted $\Theta^B_A\in\mathbb{R}^{4 \times 4}$, represents the pose of frame $A$ with respect to $B$ (i.e. $B$, in this case, is the base frame).
...
Regarding the camera, that is either placed in the environment, as in \figref{c1:fig:base2base}, or mounted at the robot end-effector, as in \figref{in:fig:eye_in_hand}, we assume that this is Stereo or RGB-D camera -- not a typical RGB camera (e.g. webcam).
...
In this sub-section we primarily describe the problem associated with \figref{c1:fig:base2base}.
...
With reference to \figref{c1:fig:base2base}, the transforms $\Theta^{R_i}_{E_i}$ are assumed known for each $i=\{1, 2\}$.
