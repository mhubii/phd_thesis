# taken from https://github.com/RViMLab/MICCAI2023-Camera-Motion-Prediction/blob/main/utils/transforms/random_edge_homography.py
...
class RandomEdgeHomography(object):
    ...
    def __init__(
        ...
        crp_shape: List[int],
        p0: float = 0.0,
        homography_return: IntEnum = HOMOGRAPHY_RETURN.DEFAULT,
    ...
    def __call__(self, img: np.array):
        ...
        top_left = self._random_top_left(
            inner_shape=self._crp_shape, outer_shape=outer_shape[:2]
        )
    ...
    def _inside(self, pts: np.array, polygon: np.array):
        ...
        if polygon.type != "Polygon":  # return false if points dont span polygon
            return False
        if (
            len(polygon.exterior.coords) != n_pts + 1
        ):  # return false should any points be the same
            return False
        for p in pts:
            p = Point(p)
